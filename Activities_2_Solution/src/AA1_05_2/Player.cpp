...
void Player::Update(std::vector<Sack*>& sacks, std::vector<Player*> players)
{
	Move(players);
	UpdateSprite();
	UpdateCollisions(sacks);
}
...
bool Player::Move(std::vector<Player*> players)
{
	dir = EDirection::NONE;
	...
	{
	case PlayerType::PL1:
		if (InputManager::getInstance()->IsPressed(InputKeys::D)) {
			newposition.x++;
			dir = EDirection::RIGHT;
		}
		if (InputManager::getInstance()->IsPressed(InputKeys::A)) {
			newposition.x--;
			dir = EDirection::LEFT;
		}
		if (InputManager::getInstance()->IsPressed(InputKeys::W)) {
			newposition.y--;
			dir = EDirection::UP;
		}
		if (InputManager::getInstance()->IsPressed(InputKeys::S)) {
			newposition.y++;
			dir = EDirection::DOWN;
		}
		break;
	case PlayerType::PL2:
		if (InputManager::getInstance()->IsPressed(InputKeys::LEFTARROW)) {
			newposition.x--;
			dir = EDirection::LEFT;
		}
		if (InputManager::getInstance()->IsPressed(InputKeys::RIGHTARROW)) {
			newposition.x++;
			dir = EDirection::RIGHT;
		}
		if (InputManager::getInstance()->IsPressed(InputKeys::UPARROW)) {
			newposition.y--;
			dir = EDirection::UP;
		}
		if (InputManager::getInstance()->IsPressed(InputKeys::DOWNARROW)) {
			newposition.y++;
			dir = EDirection::DOWN;
		}
		break;
	...
	if (newposition.x + position.w > InputManager::getInstance()->GetScreenSize()->x || newposition.x < 0) newposition.x = position.x;
	if (newposition.y + position.h > InputManager::getInstance()->GetScreenSize()->y || newposition.y < 0) newposition.y = position.y;
...
void Player::UpdateCollisions(std::vector<Sack*>& sacks)
{
	for (Sack* s : sacks) {
